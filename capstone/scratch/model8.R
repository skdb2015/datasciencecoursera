...
NGRAM_LEFTOVERPROB_TABLE = data_4gram[, .(leftOverProb = CalculateLeftOverProbability(V1, discount)), by = prefix]

CalculateLeftOverProbability <- function(freq, discount)
{
    allFreq = sum(freq)
...
getProbability_4gram <- function(in3gramPrefix, inLastWord)
{
    ...
    {
        match_4gram = data_4gram[prefix == in3gramPrefix & word(words, -1, -1) == inLastWord]
        ...
        else
        {
            ...
            match_3gram = data_3gram[prefix == n2gramPrefix & word(words, -1, -1) == inLastWord]
            ...
            if(nrow(match_3gram) > 0)
            {
                ...
                remaining_3grams = 
                    match_PrefixOf3gram[!(word(match_PrefixOf3gram$words,-1,-1) %in% word(match_PrefixOf4gram$words,-1,-1))]
            ...
            else
            {
                ...
                match_2gram = data_2gram[prefix == n1gramPrefix & word(words, -1, -1) == inLastWord]
                ...
                if (nrow(match_2gram) > 0)
                {
                    ...
                    remaining_2grams = 
                        match_prefixOf2gram[!(word(match_prefixOf2gram$words,-1,-1) %in% words(match_PrefixOf4gram$words,-1,-1))]
                ...
                else
                {
                    ...
                    match_1gram = data_1gram[word(words, -1, -1) == inLastWord]
                    
                    remaining_1gram = 
                        match_prefixOf1gram[!(word(match_prefixOf1gram$words,-1,-1) %in% words(match_PrefixOf4gram$words,-1,-1))]
